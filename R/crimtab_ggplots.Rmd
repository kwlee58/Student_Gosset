...
평균과 표준편차를 한번에 구하려면 `mapply()` 또는 `sapply()`를 이용하는 `anonymous function`, `mean_sd()`를 작성하는 게 편함. 
이와 같은 사용자정의함수를 저장해 놓으려면 `dump()`를 이용함.
...
* 평균과 표준편차를 한번에 계산하는 여러 방법들. 주석처리한 코드를 살려서 어떤 결과가 나오는지 비교. 산출값의 구조가 무엇인지 `str()`을 이용하여 파악.
