...
```{r, data loading}
load("./crimtab_test.rda")
ls()
ls.str()
head(crimtab.long.df.noise, n = 10)
```
...
```{r, suffling}
# set.seed(113)
crimtab.shuffle.noise <- crimtab.long.df.noise[sample(1:3000), ]
head(crimtab.shuffle.noise, n = 10)
```
...
finger.sample.mean <- tapply(crimtab.shuffle.noise[, "finger"], sample.id, mean)
finger.sample.sd <- tapply(crimtab.shuffle.noise[, "finger"], sample.id, sd)
...
```{r, t stats}
sample.t <- (finger.sample.mean - mean(crimtab.long.df.noise[, "finger"]))/(finger.sample.sd/sqrt(4))
str(sample.t)
```
...
hist(sample.t, prob = TRUE, breaks = seq(-30, 30, by = 0.5), xlim = c(-6, 6), ylim = c(0, 0.5), main = "Histogram of Sample t-statistics", xlab = "Sample t-values")
